const { makeUp0 } = require('./stringNumber')
/**
 * 时间格式转换
 * @param {Number,'[object Date]'} time 时间戳或者时间对象 或者时间格式的字符串
 * @param {String} fmt 'YY-MM-DD hh:mm:ss'
 * @returns
 */

exports.dateFormat = (time:string| Date, fmt = "YY-MM-DD hh:mm:ss") => {
  const date =
    Object.prototype.toString.call(new Date(time)) === "[object Date]"
      ? new Date(time)
      : new Date();

  const year = String(date.getFullYear());
  const month = String(date.getMonth() + 1);
  const day = String(date.getDate());
  const hour = String(date.getHours());
  const min = String(date.getMinutes());
  const second = String(date.getSeconds());

  
  const newTime = fmt
    .replace(/YY/g, year)
    .replace(/Y/g, String(year).substr(2))
    .replace(/MM/g, makeUp0(month))
    .replace(/M/g, month)
    .replace(/DD/g, makeUp0(day))
    .replace(/D/g, day)
    .replace(/hh/g, makeUp0(hour))
    .replace(/h/g, hour)
    .replace(/mm/g, makeUp0(min))
    .replace(/m/g, min)
    .replace(/ss/g, makeUp0(second))
    .replace(/s/g, second);

  return newTime;
};

exports.passTime = (fromTime:number) => {
  var time:number = new Date().getTime() - fromTime;
  var m = parseInt((time / 1000 / 60).toString());
  var h = parseInt((m / 60).toString());
  var d = parseInt((h / 24).toString());
  var month = parseInt((d / 30).toString());
  var y = parseInt((month / 12).toString());
  if (y) return y + "年前";
  if (month) return month + "个月前";
  if (d) return d + "天前";
  if (h) return h + "小时前";
  if (m) return m + "分钟前";
  return y + "刚刚";
};

interface Time{
  d:number;
  h:number;
  m:number;
  s:number;
  [propName:string]:any;
}
exports.remainTime = (endTime:number) => {
  var time = endTime - new Date().getTime();
  var obj:Time = { d: 0, h: 0, m: 0, s: 0 };
  if (time > 0) {
    obj.d = Math.floor(time / 1000 / 3600 / 24);
    obj.h = Math.floor((time / 1000 / 3600) % 24);
    obj.m = Math.floor((time / 1000 / 60) % 60);
    obj.s = Math.floor((time / 1000) % 60);
  }

  for (const key in obj) {
    if (Object.hasOwnProperty.call(obj, key)) {
      const item = obj[key];
      obj[key] = makeUp0(item);
    }
  }

  return obj;
};
